    ...
    return (
        <nav className='bg-black p-4 sticky z-1 top-0 text-white'>
            <div className='container mx-auto flex justify-between items-center'>
                <a className="flex gap-3 items-center cursor-pointer" href='/'>
            ...
            <div className={`md:hidden transition-all duration-300 overflow-hidden ${isOpen ? 'max-h-40' : 'max-h-0'}`}>
                <ul className='flex gap-5 flex-col items-center p-4 space-y-2'>
                    ...
                    <li><a href="/about" className={`p-2 pr-4 pl-4 hover:text-indigo-500 transition-all duration-300 ${location.pathname=='/about' ? 'text-indigo-300' : ''}`}>Sobre Empty</a></li>
                </ul>           
            </div>         
        </nav>
    );
